...
- Kernel constraint of MaxNorm(2) + dropout layers ensuring that we rescale weights beyond 2 and sampling the number of nodes active.
- Softmax as an output activation function for exclusive categorization.
...
hd_cnn |>
    ...
    layer_dropout(rate = 0.2) |>
    layer_max_pooling_2d(pool_size = c(2, 2)) |>
    layer_conv_2d(filters = 16,kernel_size = c(3, 3), kernel_constraint = constraint_maxnorm(2), activation = 'relu') |>
    layer_dropout(rate = 0.2) |>
    layer_flatten() |>
    layer_dense(units = 16, activation = 'relu') |>
    layer_dense(units = 10, activation = 'softmax')
...
Besides, compiling with an Adam SGD optimizer (best based on previous models, also beats RMSprop), I have initialized an early stopping and checkpoint callback with a patience of 5 epochs and target of validation loss. I have used a binomial crossentropy for this problem becuase we have only two classes.
...
evaluate_generator(best_hd_cnn, valid_generator, steps = length(valid_generator$labels))
...
evaluate_generator(best_hd_cnn_aug, valid_generator, steps = length(valid_generator$labels))
...
#define checkpoint callback
checkpoint <- keras::callback_model_checkpoint(filepath = 'models/best_transfer.h5', monitor = 'val_loss', save_best_only = TRUE)
...
best_transfer <- keras::load_model_hdf5('models/best_transfer.h5')
